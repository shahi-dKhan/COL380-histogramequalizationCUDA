...
run_case() {
    ...
    ./histogram_eq input.txt > _timing.txt 2>&1
    if [ $? -ne 0 ]; then
        echo "  ERROR running histogram_eq"
...
for n in 1000 5000 10000 25000 50000 100000; do
    run_case "vary_n" "n${n}" $n 32 20 uniform
done
...
for k in 1 4 8 16 32 64 128; do
    run_case "vary_k" "k${k}" 10000 $k 20 uniform
done
...
for T in 1 5 10 20 50; do
    run_case "vary_T" "T${T}" 10000 32 $T uniform
done
...
for dist in uniform dense clustered skewed; do
    run_case "dist" "$dist" 10000 32 20 $dist
done
...
for k in 1 10 32 64 128; do
    run_case "correctness" "k${k}_uniform" 1000 $k 20 uniform 1
done
for dist in dense clustered skewed; do
    run_case "correctness" "k32_${dist}" 1000 32 20 $dist 1
done
...
echo "========================================"
run_case "large" "n100k_k128_T50"      100000 128 50 uniform
run_case "large" "n100k_k32_uniform"   100000  32 20 uniform
run_case "large" "n100k_k32_dense"     100000  32 20 dense
run_case "large" "n100k_k32_clustered" 100000  32 20 clustered
