...
combination.  The final submission is a single \texttt{a2} binary built with:
\begin{lstlisting}
nvcc -O3 -arch=sm_35 -Xcompiler -fopenmp solution.cu -o a2
\end{lstlisting}
Usage: \texttt{./a2 input.txt knn|approx\_knn|kmeans}
...
is consolidated into a single \texttt{a2} binary.  Future work could reduce
per-thread register pressure via warp-cooperative heap updates to unlock the
full shared-memory tiling benefit, or explore multi-GPU partitioning for
$N > 100\,000$.
